...
- 完成成员1前端登录页，支持账号密码登录、保存 Sa-Token、保存当前用户基础信息，并在登录后返回原目标页面。
- 完成前端路由守卫，未登录访问业务页面会跳转 `/login`，已登录访问登录页会回到首页。
- 改造主 Layout，补齐成员1动态菜单、当前用户信息展示和退出登录能力。
- 完成“用户权限管理”页面，支持用户分页查询、角色筛选、新增、编辑、删除和重置密码，并按当前角色禁用非管理员写操作。
- 完成“地块台账管理”页面，支持地块分页查询、状态汇总、新增、编辑、删除和状态流转，并按当前角色禁用非管理员写操作。
- 更新首页成员1入口，将用户权限管理和地块台账作为系统基建主入口。
- 根据最新前端路由与菜单重构，成员1入口名称由“成员1”调整为“系统基建与空间资产”。
- 首页模块卡片已统一展示五个业务域，成员1仍指向 `/sys/users`，并保留地块台账二级菜单 `/sys/plots`。
- 补齐成员2、成员4模块占位入口后，成员1主 Layout 已成为五模块统一导航外壳。

## 2026.6.30

- 新增成员1系统基建工作台 `/sys`，集中展示当前登录角色、用户账号结构、地块状态结构、权限治理视图和成员一支撑链路。
- 调整成员1主入口，顶部导航和首页模块卡片统一先进入 `/sys`，再分流到用户权限管理和地块台账管理。
- 优化登录页视觉与交互，突出“系统基建安全入口”，增加 RBAC、路由守卫、空间资产能力标签，并提供管理员/农技员演示身份快捷切换。
- 优化用户权限管理页，新增权限治理英雄区、角色快捷筛选、重置筛选入口和更清晰的用户结构指标。
- 优化地块台账管理页，新增空间资产英雄区、状态卡片快捷筛选、状态视觉区分和返回系统工作台入口。
- 统一前端基础 UI 质感，调整面板、指标卡、表格和弹窗圆角与状态反馈，使成员1页面更接近完整系统模块而非单纯 CRUD 页面。
...
- 已完成成员1前端登录页、主 Layout、路由守卫、动态菜单、用户权限管理页面、地块台账管理页面。
- 已按最新命名完成五模块导航中的成员1业务域展示：`系统基建与空间资产`。
- 已完成成员1系统基建工作台和用户/地块治理型 UI 升级。
- 已通过 `mvn -DskipTests compile` 编译验证。
- 已通过 `npm run build` 前端构建验证。
...
- 前端构建验证：在 `packages/web` 下执行 `npm run build`。
- 前端访问路径：登录页 `/login`，系统基建工作台 `/sys`，用户权限管理 `/sys/users`，地块台账管理 `/sys/plots`。
