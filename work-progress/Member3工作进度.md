...
- 根据最新前端路由与菜单重构，成员3入口名称由“成员3”调整为“IoT 孪生与 AI 算法”，并与五个业务域首页入口保持一致。
- 修复成员3前端验证时打开模块报错的问题：`IotDashboardMapper` 使用 `_utf8mb4` 十六进制字面量匹配 `在线 / 离线 / 未处理`，避免中文状态值在 SQL 字符串中乱码导致 `/iot/overview` 查询失败。
- 修复成员3数据查询兼容问题：`IotDashboardMapper`、`SensorDataMapper`、`SensorManageMapper` 移除部分 `sensor.deleted = 0` 过滤，兼容未同步逻辑删除字段的旧本地库，同时当前 `init_schema.sql` 仍保留 `sensor.deleted` 字段定义。
...
- 已完成成员3查询条件兼容修正，降低旧库缺少 `sensor.deleted` 或中文状态值编码异常导致页面报错的风险。
...
- 漏洞 2：前端已有正式登录页和路由守卫，但成员3接口封装仍保留自动登录兜底，当前属于“正式入口可用、演示兜底仍需收口”的状态。
